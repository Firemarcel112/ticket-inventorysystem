...
@section("content")
    ...
    <div class="ml-4 sm:ml-0 mb-8">
        ...
        <div id="BearbeitenMain">
            <form class="w-[90%] sm:w-[80%] md:w-[70%] lg:w-[60%] xl:w-[50%]" method="POST" action="{{ route("inventorydepartmentPost", $assetdepartment->id)}}">
                @csrf
                <x-form.input
                    label="Abteilungsname"
                    inputName="name"
                    inputType="text"
                    placeholder="Abteilungs eingeben"
                    :value="$assetdepartment->name"
                    setRequired="1"
                ></x-form.input>
                <x-form.input
